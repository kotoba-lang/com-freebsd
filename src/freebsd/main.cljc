(ns freebsd.main
  "Kotodama WASM entrypoint for the Freebsd clean-room actor (L4) — Clojure port.
  ...
  facts (`freebsd.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "freebsd")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"freebsd.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "freebsd-compat" :tier tier :entities entities} 200])
...
(defn start! [] :freebsd-compat/ready)
